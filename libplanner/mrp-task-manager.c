...
//zms
void
task_manager_build_dependency_graph_for_node (MrpTaskManager *manager)
{
	MrpTaskManagerPriv *priv;
	GList              *tasks;
	GList              *l;
...
gboolean
isOccupiedTimeTasks(MrpTask *task1,MrpTask *task2)
{
	gboolean test1,test2;
	gint compare;
	mrptime s1 = 0,f1 = 0,s2 = 0,f2 = 0 ;
	//MrpTime ts1,tf1,ts2,tf2;

	g_printf("-----------in isOccupiedTimeTasks function\n");
	if(!mrp_task_compare(task1,task2))
			return FALSE;

	s1 = mrp_task_get_start(task1);
	//mrp_time_debug_print(s1);
	s2 = mrp_task_get_start(task2);
	//mrp_time_debug_print(s2);
	f1 = mrp_task_get_finish(task1);
	//mrp_time_debug_print(f1);
	f2 = mrp_task_get_finish(task2);
	//mrp_time_debug_print(f2);
	//mrp_time2_set_epoch(ts1,s1);
	//mrp_time2_set_epoch(ts2,s2);
	//mrp_time2_set_epoch(tf1,f1);
	//mrp_time2_set_epoch(tf2,f2);
	/*test1 = (mrp_time2_compare (ts1, ts2) == -1)&&(mrp_time2_compare (tf1, ts2) == -1);
	g_printf("%d",test1);
	test2 = (mrp_time2_compare (ts1, tf2) == 1)&&(mrp_time2_compare (tf1, tf2) == 1);
	g_printf("%d",test1);*/
	g_printf("-----------out isOccupiedTimeTasks function\n");


	//gint a = mrp_time2_compare (ts1, ts2);
	//gint b = mrp_time2_compare (tf1, ts2);
	//g_printf("%d,%d",a,b);
	//g_printf("\n");
...
GList *
findOccupiedTimeTasks(MrpTask *task,GList *alltasks)
{
	GList *occupiedtimetasks = NULL;
	GList *tasklist = NULL;
	GList *l1 = NULL;
	gboolean a;
	GList *l2 = NULL;
	g_printf("-----------in findOccupiedTimeTasks function\n");
	//tasklist = mrp_project_get_all_tasks(project);
	tasklist = alltasks;
	MrpTask *t;
	for(l1 = tasklist;l1;l1 = l1->next)
	{
		if(isOccupiedTimeTasks(task,l1->data))
			{
				occupiedtimetasks = g_list_append(occupiedtimetasks,l1->data);
			}
	}
	g_printf("-----------out findOccupiedTimeTasks function\n");
	return occupiedtimetasks;
}

gint
findMostOccupiedTask(GList *tasklist)
{
	MrpTask *t;
	GList *l1 = NULL;
	GList *l2 = tasklist;
	gint n = g_list_length(tasklist);
	gint number[n];
	gint i = -1;
	gint k;
	gint most = 0;
	...
	{
		i++;
		l2 = findOccupiedTimeTasks(l1->data,tasklist);
		number[i] = g_list_length(l2);
	}
	for(k=0;k<n;k++)
		{
			if(number[k]>most)
...
GList *
isNotOccupiedTask(GList *tasklist)
{
	GList *l1 = NULL;
	GList *l2 = NULL;
	GList *l3 = NULL;
	GList *nooccup = g_list_copy(tasklist);
	for(l1 = tasklist;l1;l1 = l1->next)
	{
		MrpTask *t = l1->data;
		for(l2 = tasklist;l2;l2 = l2->next)
		{
			if(isOccupiedTimeTasks(t,l2->data))
			{
				nooccup = g_list_remove(nooccup,t);
				break;
			}
		}
	}
...
gint
theTaskResourceNumber(MrpTask *task)
{
	gint number;
	GList *l = NULL;
	l = mrp_task_get_assignments(task);
	number = g_list_length(l);
	return number;
}

MrpTask *
findMinResource(GList *tasklist)
{
	GList *l;
	gint i = -1;
	gint j,k,m;
	...
	for(l=tasklist;l;l=l->next)
	{
		i++;
		j = theTaskResourceNumber(l->data);
		//k = theTaskResourceNumber(l->next->data);
		number[i] = j;
	}
	for(k=0;k<n;k++)
	{
	...
	return g_list_nth_data(tasklist,mintask);
}

gboolean
resourceNotEnough(MrpProject *project)
{
	GList *resources = mrp_project_get_resources(project);
	GList *tasklist = mrp_project_get_all_tasks(project);
	gint resnumber = g_list_length(resources);
	gint neednumber = findMostOccupiedTask(tasklist);
	if(resnumber < neednumber)
		return TRUE;
	else
		return FALSE;
}
...
gint
sortTasklistsFunction(gconstpointer *t1,gconstpointer *t2)
{
	mrptime s1 = 0,s2 = 0;
	s1 = mrp_task_get_start(MRP_TASK(t1));
	s2 = mrp_task_get_start(MRP_TASK(t2));
	if(s1 < s2)
		return -1;
	else
		if(s1 == s2)
...
GList *
sortTasklistsByStartTime(GList *tasklist)
{
	GList *l = NULL;
	l = g_list_sort(tasklist,sortTasklistsFunction);
	return l;
}

gint
sortTasklistsFunctionByFinish(gconstpointer *t1,gconstpointer *t2)
{
	mrptime s1 = 0,s2 = 0;
	mrptime s3,s4;
	s1 = mrp_task_get_finish(MRP_TASK(t1));
	s2 = mrp_task_get_finish(MRP_TASK(t2));

	s3 = mrp_task_get_start(MRP_TASK(t1));
	s4 = mrp_task_get_start(MRP_TASK(t2));
	g_print("%d ,%d,%d ,%d",s1,s2,s3,s4);
	if(s1 < s2)
		return -1;
	else
		if(s1 == s2)
...
GList *
sortTasklistsByFinishTime(GList *tasklist)
{

	GList *l = NULL;
	l = g_list_sort(tasklist,sortTasklistsFunctionByFinish);
	return l;
}
...
mrptime
totalduration(MrpProject *project)
{
	mrptime totaltime = 0;
	MrpTaskManager *task_manager = imrp_project_get_task_manager(project);
	GList *tasklist = mrp_task_manager_get_all_tasks(task_manager);
	GList *criticaltasks = getCriticalTasks(tasklist);
	GList *l = criticaltasks;
	mrptime starttime = mrp_task_get_start(l->data);
	mrptime finishtime;
	MrpTask *lasttask;
	gint second = 60*60*8;
	for(;l;l=l->next)
	...
	finishtime = mrp_task_get_finish(lasttask);
	totaltime = (finishtime - starttime)/second;
	return totaltime;
}

GList *
getCriticalTasks(GList *tasklist)
{
	GList *l = NULL;
	GList *criticallist = NULL;
	GList *l2 = NULL;
	gint iscritical;
	for(l=tasklist;l;l=l->next)
	{
		iscritical = mrp_task_get_critical(l->data);
		if(iscritical)
		{
			criticallist = g_list_append(criticallist,l->data);
		}
	}
	l2 = criticallist;
	criticallist = sortTasklistsByStartTime(l2);
	return criticallist;
}


GList *
findAllAssignmentRresource(MrpProject *project)
{
	GList *l1 = NULL;
	GList *l2 = NULL;
	GList *l3 = NULL;
	...
	tasklist = mrp_task_manager_get_all_tasks(task_manager);
	for(n=0;n<g_list_length(resources);n++)
	{
		number[n] = 0;
	}
	for(l1=tasklist;l1;l1=l1->next)
	{
		lr = mrp_task_get_assigned_resources (l1->data);
		lrtotal = g_list_concat(lrtotal,lr);
	}
	/*l3=g_list_copy(lrtotal);

		for(l2=resources;l2;l2=l2->next)
		{

			l4 = l3;
			gchar *s1 = mrp_resource_get_name (l2->data);
			while(l4)
			{
				gchar *s2 = mrp_resource_get_name (l4->data);
				if(!g_strcmp0(s1,s2))
				{
					del++;
					l3 = g_list_remove(l3,l4->data);
					l4 = l3;
				}
				else
				{
		...
		l5 = resources;
		for(j=0;j<g_list_length(resources);j++)
			{
			if(number[j] == 1)
				l6 = g_list_append(l6,l5->data);
			l5	= l5->next;
			}
*/
	return lrtotal;
...
GList *
findNotAssignmentRresource(GList *resources,GList *hasresource)
{
	GList *l1 = NULL;
	GList *l2 = NULL;
	GList *lnot= NULL;
	...
	MrpTask *opttask;
	MrpResource *resource;
	lnot = g_list_copy(resources);

	for(l1=resources;l1;l1=l1->next)
	{
		for(l2=hasresource;l2;l2=l2->next)
			{
				gchar *s1 = mrp_resource_get_name (l1->data);
				gchar *s2 = mrp_resource_get_name (l2->data);
				if(!g_strcmp0(s1,s2))
				{
					lnot = g_list_remove(lnot,l2->data);
				}
			}
...
void
durationOptimize(MrpProject *project)
{
	GList *l1 = NULL;
	GList *l2 = NULL;
	GList *l3 = NULL;
	...
	gint i,j,min,n;
	task_manager = imrp_project_get_task_manager(project);
	priv = task_manager->priv;
	priv->needs_recalc = TRUE;
	priv->in_recalc = TRUE;
	tasklist = mrp_task_manager_get_all_tasks(task_manager);
	l1 = getCriticalTasks(tasklist);
	i = 0;
	n = 0;
	for(;l1;l1=l1->next)
	{
		ren[i] = theTaskResourceNumber(l1->data);
		i++;
	}

	min = 1000;
	for(j=0;j<g_list_length(l1);j++)
	{
		if(ren[j] < min)
		{
			min = ren[j];
	...
	opttask = g_list_nth_data(l1,n);
	l2 = findAllAssignmentRresource(project);
	l3 = findNotAssignmentRresource(resources,l2);
	if(opttask)
		mrp_resource_assign (l3->data,opttask,100);
	else
		g_printf("task is null\n");
	while(l3 && opttask)
	{
		priv->needs_recalc = TRUE;
		priv->in_recalc = TRUE;
		l1 = getCriticalTasks(tasklist);
			...
			l5 = l3;
			if(opttask)
				tr = mrp_task_get_assigned_resources(opttask);
			for(l4=tr;l4;l4=l4->next)
						{
							gchar *s1 = mrp_resource_get_name (l5->data);
							gchar *s2 = mrp_resource_get_name (l4->data);
							if(!g_strcmp0(s1,s2))
							{
								//lnot = g_list_remove(lnot,l2->data);
			...
			if(l5->data && opttask)
				mrp_resource_assign (l5->data,opttask,100);
	}
