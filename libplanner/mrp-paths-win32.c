...
mrp_paths_get_image_dir (const gchar *filename)
{
	if (!image_dir) {
		gchar *module_dir;
		module_dir = g_win32_get_package_installation_directory_of_module (NULL);
		image_dir = g_build_filename (module_dir, IMAGEDIR, NULL);
		g_free (module_dir);
	}
...
mrp_paths_get_plugin_dir (const gchar *filename)
{
	if (!plugin_dir) {
		gchar *module_dir;
		module_dir = g_win32_get_package_installation_directory_of_module (NULL);
		plugin_dir = g_build_filename (module_dir, PLUGINDIR, NULL);
		g_free (module_dir);
	}
...
mrp_paths_get_dtd_dir (const gchar *filename)
{
	if (!dtd_dir) {
		gchar *module_dir;
		module_dir = g_win32_get_package_installation_directory_of_module (NULL);
		dtd_dir = g_build_filename (module_dir, DTDDIR, NULL);
		g_free (module_dir);
	}
...
mrp_paths_get_stylesheet_dir (const gchar *filename)
{
	if (!stylesheet_dir) {
		gchar *module_dir;
		module_dir = g_win32_get_package_installation_directory_of_module (NULL);
		stylesheet_dir = g_build_filename (module_dir, STYLESHEETDIR, NULL);
		g_free (module_dir);
	}
...
mrp_paths_get_ui_dir (const gchar *filename)
{
	if (!ui_dir) {
		gchar *module_dir;
		module_dir = g_win32_get_package_installation_directory_of_module (NULL);
		ui_dir = g_build_filename (module_dir, UIDIR, NULL);
		g_free (module_dir);
	}
...
mrp_paths_get_storagemodule_dir (const gchar *filename)
{
	if (!storagemodule_dir) {
		gchar *module_dir;
		module_dir = g_win32_get_package_installation_directory_of_module (NULL);
		storagemodule_dir = g_build_filename (module_dir, STORAGEMODULEDIR, NULL);
		g_free (module_dir);
	}
...
mrp_paths_get_file_modules_dir (const gchar *filename)
{
	if (!file_modules_dir) {
		gchar *module_dir;
		module_dir = g_win32_get_package_installation_directory_of_module (NULL);
		file_modules_dir = g_build_filename (module_dir, FILEMODULEDIR, NULL);
		g_free (module_dir);
	}
...
mrp_paths_get_sql_dir (void)
{
	if (!sql_dir) {
		gchar *module_dir;
		module_dir = g_win32_get_package_installation_directory_of_module (NULL);
		sql_dir = g_build_filename (module_dir, SQLDIR, NULL);
		g_free (module_dir);
	}
...
mrp_paths_get_locale_dir (void)
{
	if (!locale_dir) {
		gchar *module_dir;
		module_dir = g_win32_get_package_installation_directory_of_module (NULL);
		locale_dir = g_build_filename (module_dir, GNOMELOCALEDIR, NULL);
		g_free (module_dir);
	}
