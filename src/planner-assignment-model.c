		...
		GValue       *value)
{
	...
	resource_object = MRP_OBJECT (G_LIST(iter->user_data)->data);
	if(MRP_IS_RESOURCE(resource_object)) {
		...
		resource_group = MRP_GROUP(resource_object);
		switch (column) {
		case RESOURCE_ASSIGNMENT_COL_NAME:
			g_object_get (resource_group, "name", &str, NULL);
			if (str == NULL) {
				str = g_strdup ("");
			}
			g_value_init (value, G_TYPE_STRING);
		...
		case RESOURCE_ASSIGNMENT_COL_ASSIGNED:
			g_value_init (value, G_TYPE_BOOLEAN);
			if (mrp_task_get_group_assignment (model->priv->task, resource_group)) {
				g_value_set_boolean (value, TRUE);
			} else {
				g_value_set_boolean (value, FALSE);
			}
		...
		case RESOURCE_ASSIGNMENT_COL_ASSIGNED_UNITS:
			g_value_init (value, G_TYPE_INT);
			assignment = mrp_task_get_group_assignment (model->priv->task, resource_group);
			if (assignment) {
				g_value_set_int (value, mrp_group_assignment_get_units (assignment));
			} else {
				g_value_set_int (value, 0);
			}
			break;
...
planner_assignment_model_new (MrpTask *task)
{
	...
	resources = g_list_copy(resources);
	resources = g_list_concat(resources, g_list_copy(mrp_project_get_groups (priv->project)));
