...
static void
task_tree_probability_data_func (GtkTreeViewColumn *tree_column,
			  GtkCellRenderer   *cell,
			  GtkTreeModel      *tree_model,
			  GtkTreeIter       *iter,
			  gpointer           data)
{
	...
	str = planner_format_int (probability);
...
static void
task_tree_probability_edited (GtkCellRendererText *cell,
		       gchar               *path_string,
		       gchar               *new_text,
		       gpointer             data)
{
	...
	probability = atoi(new_text);

	if (probability < 0)   probability = 0;
	if (probability > 100) probability = 100;

	if (mrp_task_get_probability (MRP_TASK (task)) != probability) {
		g_value_init (&value, G_TYPE_INT);
		g_value_set_int (&value, probability);
		      ...
		      const gchar *title)
{
	...
	case COL_PROBABILITY:
			...
			gtk_tree_view_column_set_cell_data_func (col,
								 cell,
								 task_tree_probability_data_func,
								 GTK_TREE_VIEW (tree),
								 NULL);
			g_object_set_data (G_OBJECT (col),
					   "data-func", task_tree_probability_data_func);
			g_object_set_data (G_OBJECT (col), "id", "probability");

			g_signal_connect (cell,
					  "edited",
					  G_CALLBACK (task_tree_probability_edited),
					  GTK_TREE_VIEW (tree));
